...
public class SandwichSubmersible extends Submersible {
	...
	boolean inengineplaying;
public SandwichSubmersible(SubAbyss game,AssetManager assman){
	game.assman.manager.finishLoading();
	game.assman.manager.load("data/sub.png",Texture.class);
	game.assman.manager.load("data/subpropeller.png",Texture.class);
	game.assman.manager.finishLoading();
	
	text=game.assman.manager.get("data/sub.png",Texture.class);
	text.setFilter(TextureFilter.Linear, TextureFilter.Linear);
	if(assman.update()) {
	subshopsprite=new Sprite(text);
	propeller=new Sprite(game.assman.manager.get("data/subpropeller.png",Texture.class));}
}
public SandwichSubmersible(Batch batch, World world,float statetime, OrthographicCamera camera, AssetManager assman, final SubAbyss game){
	...
	subtext=game.assman.manager.get("data/sub.png",Texture.class);

	bubble=game.assman.manager.get("data/bubble.png",Texture.class);
	proptext=game.assman.manager.get("data/subpropeller.png",Texture.class);
    ...
    float[] subverts= {-0.9f,-0.5f,-0.9f,-0.2f,-0.75f,0.3f,0.75f,0.3f,0.9f,0.1f,0.9f,-0.5f};
    subbox.set(subverts);
	...
	float[] verticies=new float[] {-0.1f,0,0.4f,0,0.4f,0.6f,-0.1f,0.6f};
	subbox2.set(verticies); 
    FixtureDef subfdhat = new FixtureDef();
	 subfdhat.shape = subbox2;
	 subfdhat.friction=3f;
	 subfdhat.density=0.3f;
...
//	if(launcherequipped) {
//		table.add(launch).pad(5f).center().bottom().padBottom(50f).fillX();
//	}
...
//	
	...
	if(!subhit){
	 propeller.setPosition((subbody.getPosition().x*SubAbyss.BOX_WORLD_TO-120f), subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-30f);
	}
