...
public class ShinkaiSubmersible extends Submersible {
	...
	int score=0;
	Tool toolarm;
	String subname="DSV Shinkai 6500";
	...
	float topspeed=1.4f;
	ConeLight dl,dl2;
	PointLight pl,pl3,pl4;
	int strength=200;
	Texture forwardwalkbutton,sideway;
	float joystick,statetime;
	Sprite forwardsprite,backwardsprite,upsprite,downsprite,menusprite;
	Texture subtext,bubble,proptext,arm1,arm2,sublighttext,menuicon;
	...
	Sprite subsprite,bubblesprite,leftsprite,rightsprite,propeller,sublight;
	World world;
	boolean launcherequipped;
	Sprite launchsprite;
	boolean right,up,downboo,upboo;
	boolean backward;
	private long lastlight;
	Image launch,forwardwalk,menu;
	private boolean lighton;
public ShinkaiSubmersible(SubAbyss game,AssetManager assman){
	this.game=game;
	game.assman.manager.load("data/sub3.png",Texture.class);
	game.assman.manager.load("data/subpropeller2.png",Texture.class);
	//assman.update();
	game.assman.manager.finishLoading();
	if(assman.update()) {
	Texture shinkai=game.assman.manager.get("data/sub3.png",Texture.class);
//	shinkai.setFilter(TextureFilter.Nearest,TextureFilter.Linear);
	shinkai.setFilter(TextureFilter.Linear, TextureFilter.Linear);
	subshopsprite=new Sprite(shinkai);
	propeller=new Sprite(game.assman.manager.get("data/subpropeller2.png",Texture.class));}
}
public ShinkaiSubmersible(Batch batch, World world,float statetime, OrthographicCamera camera, AssetManager assman, final SubAbyss game){
//HI
	this.camera=camera;
	this.batch=batch;
	this.world=world;
	...
	subtext=game.assman.manager.get("data/sub3.png",Texture.class);
    ...
    float[] subverts= {-0.6f,0.3f,-1.41f,-0.2f,1.3f,0.4f,1.35f,0f,0.8f,-0.55f,-0.3f,-0.55f};
    subbox.set(subverts);
   // subbox.setAsBox((subsprite.getWidth()/4)*SubAbyss.WORLD_TO_BOX,(subsprite.getHeight()/2)*SubAbyss.WORLD_TO_BOX);
	...
	if(game.storage.getChosenTool().getToolName().equals("Basic Arm")) {
		...
		toolarm=new Phaser(80f*SubAbyss.WORLD_TO_BOX, -35f*SubAbyss.WORLD_TO_BOX, subbody, game);
			...
			toolarm=new SpudGun(80f*SubAbyss.WORLD_TO_BOX, -45f*SubAbyss.WORLD_TO_BOX, subbody, game);
	...
	dl=new ConeLight(game.level.rayhandler, 20, Color.TEAL, 5f, 0, 0, 9f, 30f);
	dl.attachToBody(subbody,(subsprite.getWidth()/4+12f)*game.WORLD_TO_BOX,-40f*game.WORLD_TO_BOX);
	dl.setActive(false);
	
	dl2=new ConeLight(game.level.rayhandler, 20, Color.TEAL, 3f, 0, 0, 9f, 20f);
	dl2.attachToBody(subbody,(subsprite.getWidth()/4-10f)*game.WORLD_TO_BOX,-50f*game.WORLD_TO_BOX,-45f);
	dl2.setActive(false);
	
	
	
	pl=new PointLight(game.level.rayhandler, 25, Color.WHITE, 0.25f,0,0);
	pl.attachToBody(subbody,(subsprite.getWidth()/4+13f)*game.WORLD_TO_BOX,-26*game.WORLD_TO_BOX);
...
public void draw(){
	...
	if(!subhit){
	 propeller.setPosition((subbody.getPosition().x*SubAbyss.BOX_WORLD_TO-155f), subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-41f);
	}
	else{
	 ...
	 if(Gdx.input.isKeyPressed(Keys.RIGHT)||right||joystick>0.65){
		 ...
		 Level1.bubbles.add(new Bubble(new Sprite(bubblesprite),game, 1f,world,batch,(subbody.getPosition().x*SubAbyss.BOX_WORLD_TO)-220f,subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-14f,5f,false));
		if(subbody.getLinearVelocity().x<2.0f){
		   subbody.applyForceToCenter(new Vector2 (3.9f,0), true);}
		else{
		   subbody.setLinearVelocity(2.0f, subbody.getLinearVelocity().y);}
	 ...
	 if(subbody.getLinearVelocity().x>0) {
	   ...
	   if(Gdx.input.isKeyPressed(Keys.DOWN)||downboo){
		 ...
		 if(subbody.getLinearVelocity().y>-3.0f){
			 ...
			 subbody.setLinearVelocity(subbody.getLinearVelocity().x,-3.0f);
		 }
		   if(propeller.getRotation()>-30f){
			   propeller.rotate(-2f);}
	   }
	   if(Gdx.input.isKeyPressed(Keys.UP)||upboo){
		   bubblesprite.setScale(-MathUtils.random(0.2f,1f));
		   Level1.bubbles.add(new Bubble(new Sprite(bubblesprite),game, 1f,world,batch,(subbody.getPosition().x*SubAbyss.BOX_WORLD_TO)-220f,subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-14f,5f,false));
	   ...
	   if(depth<=0) {
		   ...
		   toolarm.getArm1Body().setGravityScale(10f);
		   toolarm.getArm2Body().setGravityScale(10f);
	   }
	   ...
	   if(Gdx.input.isKeyPressed(Keys.LEFT)||backward||joystick<-0.65){
		   bubblesprite.setScale(-MathUtils.random(0.2f,1f));
		   Level1.bubbles.add(new Bubble(new Sprite(bubblesprite),game, 1f,world,batch,(subbody.getPosition().x*SubAbyss.BOX_WORLD_TO)-220f,subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-14f,5f,false));
		   if(subbody.getLinearVelocity().x>-3.0f){
			   subbody.applyForceToCenter(new Vector2 (-3.9f,0), true);}
			else{
	   ...
	   if((right||backward||up||downboo)&&Level1.bubbles.size<50){
		   bubblesprite.setScale(-MathUtils.random(0.2f,1f));
		   Level1.bubbles.add(new Bubble(new Sprite(bubblesprite),game, 1f,world,batch,(subbody.getPosition().x*SubAbyss.BOX_WORLD_TO)-210f,subbody.getPosition().y*SubAbyss.BOX_WORLD_TO-2f,5f,false));
	   }
...
public void hit(int hitpoints){
	...
	Timer.schedule(new Task(){
        ...
        public void run() {
        	...
        	toolarm.getArm2Sprite().setColor(Color.RED);
        	toolarm.getArm1Sprite().setColor(Color.RED);
...
public void activateArm(Fish fs){
	toolarm.activateArm(fs);
...
public void restoreAngle(){
	//subbody.setFixedRotation(false);
	
	System.out.println("Sub angle "+normalizeAngle(subbody.getAngle()*MathUtils.radDeg));
while(true){
	if(!world.isLocked()){
...
break;
	...
	//if(normalizeAngle(subbody.getAngle()*MathUtils.radiansToDegrees)<1f&&normalizeAngle(subbody.getAngle()*MathUtils.radiansToDegrees)>-1f){
...
public boolean isLocked() {
	...
	return game.keeper.isShinkaiLocked();
}
