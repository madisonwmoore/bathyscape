...
public class ChopstickArm extends Tool {
String type = "Chopstick Arm";
String description="Great for fish, bad for soup.";
Body armbody,arm2body;
TextureRegion[] armframe;
SpriteBatch batch;
Sprite arm1sprite,arm2sprite,arm2spriteclosed,arm2spriteopen, chopstick1,chopstick2;
SubAbyss game;
...
boolean isarmplaying;
	public ChopstickArm(SubAbyss game) {
		
		Texture arm1=game.assman.manager.get("data/arm1.png",Texture.class);
		Texture arm3=game.assman.manager.get("data/arm3.png",Texture.class);
		//Texture chopstick=game.assman.manager.get("data/chopstick.png",Texture.class);
		 arm1sprite=new Sprite(arm1);
		 
	// chopstick1=new Sprite(chopstick);
		// chopstick2=new Sprite(chopstick);
		 TextureRegion[][] tmp = TextureRegion.split(arm3,arm3.getWidth() / 
					1, arm3.getHeight() / 2);                                
					        armframe = new TextureRegion[1 * 2];
					        int index1 = 0;
					        for (int i = 0; i < 2; i++) {
					                for (int j = 0; j < 1; j++) {
					                        armframe[index1++] = tmp[i][j];
					                }
					        }
					        
				arm2sprite=new Sprite(armframe[0]);	        
	...
	public ChopstickArm(float x, float y, Body subbody, SubAbyss game) {
		this.batch=game.level.batch;
		this.game=game;
		Texture arm1=game.assman.manager.get("data/arm1.png",Texture.class);
		Texture arm2=game.assman.manager.get("data/arm2.png",Texture.class);
		Texture chopstick=game.assman.manager.get("data/arm3.png",Texture.class);
		

		TextureRegion[][] tmp = TextureRegion.split(chopstick,chopstick.getWidth() / 
				1, chopstick.getHeight() / 2);                                
				        armframe = new TextureRegion[1 * 2];
				        int index1 = 0;
				        for (int i = 0; i < 2; i++) {
				                for (int j = 0; j < 1; j++) {
		...
		arm1sprite=new Sprite(arm1);
		 arm2sprite=new Sprite(armframe[0]);
		 arm2spriteopen=new Sprite(armframe[0]);
		 arm2spriteclosed=new Sprite(armframe[1]);
	//	 chopstick1=new Sprite(chopstick);
	//	 chopstick2=new Sprite(chopstick);
		...
		armarm2swing.localAnchorB.x=-27f*SubAbyss.WORLD_TO_BOX;
	...
	public void draw() {
//			chopstick1.setPosition(arm2body.getPosition().x*SubAbyss.BOX_WORLD_TO-(arm2sprite.getWidth()/2)+10f,arm2body.getPosition().y*SubAbyss.BOX_WORLD_TO-(arm2sprite.getHeight()/2)+10f);
//			chopstick1.setOrigin(arm2sprite.getOriginX(),arm2sprite.getOriginY());
//			
//			chopstick1.setRotation(arm2body.getAngle()*MathUtils.radiansToDegrees-15f);
//			chopstick1.draw(batch);
		
//			chopstick2.setPosition(arm2body.getPosition().x*SubAbyss.BOX_WORLD_TO-arm2sprite.getWidth()/2+50f,arm2body.getPosition().y*SubAbyss.BOX_WORLD_TO-(arm2sprite.getHeight()/2+50f));
//			chopstick2.setOrigin(-20f,(arm2sprite.getHeight()/2+50f));
//			chopstick2.setRotation(arm2body.getAngle()*MathUtils.radiansToDegrees+45f);
//			chopstick2.draw(batch);
	...
	public void activateArm(final Fish tf){
		...
		if(!armrunning) {
			arm2sprite.set(arm2spriteclosed);;
